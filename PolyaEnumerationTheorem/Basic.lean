...
instance [Fintype X] [DecidableEq Y] [Fintype G] :
  ...
  fun f₁ f₂ ↦
    if h : ∃ g, g • f₂ = f₁ -- Can be decided because `G`, `X` are finite and `Y` has decidable equality.
    then isTrue h
    else isFalse h
...
instance {f : Equiv.Perm X} [Fintype X] [DecidableEq X] :
  DecidableRel (@HasEquiv.Equiv X (@instHasEquivOfSetoid X (Equiv.Perm.SameCycle.setoid f))) :=
  Equiv.Perm.instDecidableRelSameCycleOfFintypeOfDecidableEq f
...
abbrev numCyclesOfGroup {G : Type v} [Group G] [MulAction G X] (g : G) [Fintype (CyclesOfGroup X g)] : ℕ :=
...
-- #eval numCyclesOfGroup (Fin 3) (c[0, 1] : Equiv.Perm (Fin 3)) -- Group of permutations of `{0, 1, 2}` acts on `{0, 1, 2}`. Has two cycles.

/-- The number of elements in the group `G` that have exactly `i` cycles. -/
abbrev numGroupOfNumCycles (G : Type v) [Group G] [MulAction G X] [Fintype G] [∀ g : G, Fintype (CyclesOfGroup X g)] (i : ℕ) : ℕ :=
...
def equiv_of_fixedBy_coloring_of_cycle_coloring (g : G) : (CyclesOfGroup X g → Y) ≃ (MulAction.fixedBy (X → Y) g) :=
...
variable (X : Type u) (Y : Type v) (G : Type w) [Group G] [MulAction G X] [Fintype Y] [Fintype G]
  [Fintype (Quotient (MulAction.orbitRel G (X → Y)))] [∀ g : G, Fintype (MulAction.fixedBy (X → Y) g)]
...
theorem numDistinctColorings_mul_card_group_eq_sum_card_pow_numCyclesOfGroup [∀ g : G, Fintype (CyclesOfGroup X g)]
  [∀ g : G, DecidableEq (CyclesOfGroup X g)] :
  (numDistinctColorings X Y G) * (Fintype.card G) = ∑ g : G, (Fintype.card Y) ^ (numCyclesOfGroup X g) := by
...
/-- *Pólya's enumeration theorem*:
    ...
    To use this theorem it suffices to have: `[Group G] [MulAction G X] [Fintype X] [Fintype Y] [Fintype G] [DecidableEq X] [DecidableEq Y]`. -/
theorem numDistinctColorings_eq_sum_card_pow_numCyclesOfGroup_div_card_group [∀ g : G, Fintype (CyclesOfGroup X g)]
  [∀ g : G, DecidableEq (CyclesOfGroup X g)] :
  numDistinctColorings X Y G = (∑ g : G, (Fintype.card Y) ^ (numCyclesOfGroup X g)) / (Fintype.card G) := by
  exact Nat.eq_div_of_mul_eq_left Fintype.card_ne_zero (numDistinctColorings_mul_card_group_eq_sum_card_pow_numCyclesOfGroup X Y G)
...
theorem numDistinctColorings_mul_card_group_eq_sum_numGroupOfNumCycles_mul_card_pow [Fintype X]
  [∀ g : G, DecidableRel (@HasEquiv.Equiv X (@instHasEquivOfSetoid X (Equiv.Perm.SameCycle.setoid (MulAction.toPerm g))))] :
  (numDistinctColorings X Y G) * (Fintype.card G) = ∑ i : Fin (Fintype.card X + 1), (numGroupOfNumCycles X G i.1) * (Fintype.card Y) ^ i.1 := by
  rw [numDistinctColorings_mul_card_group_eq_sum_card_pow_numCyclesOfGroup]
  calc
    ∑ g : G, (Fintype.card Y) ^ (numCyclesOfGroup X g) =
      ∑ i : Fin (Fintype.card X + 1), ∑ _ ∈ {g : G | numCyclesOfGroup X g = i}, (Fintype.card Y) ^ i.1 := by
        ...
        · intro _ hg
          simp at hg
          rw [hg]
    _ = ∑ i : Fin (Fintype.card X + 1), (numGroupOfNumCycles X G i.1) * (Fintype.card Y) ^ i.1 := by simp
